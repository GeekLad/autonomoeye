...
def write_annotations(model, images_path, write_path, nms_thresh, score_thresh, device):
    ...
    for filename in image_files:
        ...
        labels = [int(x.detach().to('cpu'))
                  for idx, x in enumerate(labels) if idx in keep]
        bbox = [x.detach().to('cpu')
                for idx, x in enumerate(bbox) if idx in keep]
        scores = [x.detach().to('cpu')
                  for idx, x in enumerate(scores) if idx in keep]
        ...
        i = 0
        # Filter for scores greater than certain threshold
        scores_ind = [idx for idx, x in enumerate(scores) if x > score_thresh]
        for idx, entry in enumerate(bbox):
            if idx in scores_ind:
                h = entry[2] - entry[0]
                w = entry[3] - entry[1]
                ...
                # Create a Rectangle patch
                rect = patches.Rectangle((entry[0], entry[1]), h, w, linewidth=4, edgecolor=colors_map[str(
                    labels[idx])], facecolor='none')

                # Add classification category
                plt.text(entry[0], entry[1], s=labels_map[str(labels[idx])],
                         color='white', verticalalignment='top',
                         bbox={'color': colors_map[str(
                             labels[idx])], 'pad': 0},
                         font={'size': 25})
